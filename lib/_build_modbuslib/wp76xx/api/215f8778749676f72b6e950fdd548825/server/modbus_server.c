...
)
{
    ...
    _ServerDataPool = le_mem_CreatePool("modbus_ServerData", sizeof(_ServerData_t));

    // Create safe reference map for handler references.
    // The size of the map should be based on the number of handlers defined for the server.
    // Don't expect that to be more than 2-3, so use 3 as a reasonable guess.
    _HandlerRefMap = le_ref_CreateMap("modbus_ServerHandlers", 3);
...
static void Handle_modbus_SetParameter
(
...
static void Handle_modbus_GetParameter
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    {
        LE_DEBUG("Adjusting valueSize from %zu to 100", valueSize);
        valueSize = 100;
    }
...
static void Handle_modbus_Connect
(
    le_msg_MessageRef_t _msgRef
...
static void Handle_modbus_Disconnect
(
    le_msg_MessageRef_t _msgRef
...
static void Handle_modbus_Read_Bit
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    if ( (destSize > 128) )
    {
        LE_DEBUG("Adjusting destSize from %zu to 128", destSize);
        destSize = 128;
    }
    ...
    if (destPtr)
    {
        ...
        LE_PACK_PACKARRAY( &_msgBufPtr, &_msgBufSize,
                           destPtr, (*destSizePtr),
                           128, le_pack_PackUint8,
                           &destResult );
        LE_ASSERT(destResult);
    }
...
static void Handle_modbus_Read_InputBit
(
    le_msg_MessageRef_t _msgRef
...
static void Handle_modbus_Read_Word
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    // Define storage for output parameters
    uint16_t destBuffer[128];
    uint16_t *destPtr = destBuffer;
    size_t *destSizePtr = &destSize;
    if (!(_requiredOutputs & (1u << 0)))
    {
        destPtr = NULL;
...
static void Handle_modbus_Read_InputWord
(
    le_msg_MessageRef_t _msgRef
...
static void Handle_modbus_Write_Bit
(
    le_msg_MessageRef_t _msgRef
...
static void Handle_modbus_Write_Word
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_modbus_SetParameter : Handle_modbus_SetParameter(msgRef); break;
        case _MSGID_modbus_GetParameter : Handle_modbus_GetParameter(msgRef); break;
        case _MSGID_modbus_Connect : Handle_modbus_Connect(msgRef); break;
        case _MSGID_modbus_Disconnect : Handle_modbus_Disconnect(msgRef); break;
        case _MSGID_modbus_Read_Bit : Handle_modbus_Read_Bit(msgRef); break;
        case _MSGID_modbus_Read_InputBit : Handle_modbus_Read_InputBit(msgRef); break;
        case _MSGID_modbus_Read_Word : Handle_modbus_Read_Word(msgRef); break;
        case _MSGID_modbus_Read_InputWord : Handle_modbus_Read_InputWord(msgRef); break;
        case _MSGID_modbus_Write_Bit : Handle_modbus_Write_Bit(msgRef); break;
        case _MSGID_modbus_Write_Word : Handle_modbus_Write_Word(msgRef); break;
